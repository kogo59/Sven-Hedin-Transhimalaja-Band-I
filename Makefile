BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = Transhimalaja_Sven_Hedin_Erster_Band
TITLE_NAME = "Transhimalaja. Erster Band."
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Transhimalaja_Band_I_*.jpg .
